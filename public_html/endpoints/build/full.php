...
if(defined("entrypoint") == true)
{
    ...
    if(file_exists("../required/csv_dataset/tmp/name2key.csv") == false)
    {
        ...
        {
            if(file_exists("../required/csv_dataset/group_a.sql") == false)
            {
    ...
    if($all_ok == true)
    {
        if(file_exists("../required/csv_dataset/group_a.sql") == false)
        {
            ...
            if ($handle)
            {
                ...
                while (($line = fgets($handle)) !== false)
                {
                    ...
                    if(count($bits) == 2)
                    {
                        ...
                        $use_group = "other";
                        if(array_key_exists($firstbit,$group_options) == true)
                        {
                            $use_group = $firstbit;
                        }
                        ...
                        if(($group_data[$use_group]["next_id"]%200)==0)
                        {
                ...
                foreach(array_keys($group_data) as $key)
                {
                    ...
                    echo "Group ".$key." has ".($group_data[$use_group]["next_id"]-1)." entrys<br/>";
                    $file = "../required/csv_dataset/group_".$key.".sql";
            ...
            foreach($group_options as $option)
            {
                set_time_limit (480);
                output("group ".$option." -");
                $sql->RawSQL("../required/csv_dataset/group_".$option.".sql");
                output(" DONE <br/>");
            }
            $sql->sqlSave();
